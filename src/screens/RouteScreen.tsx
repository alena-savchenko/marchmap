...
export function RouteScreen() {
    ...
    <View style={styles.toolbar}>
      <Image accessibilityLabel="MarchMap" source={require('../../assets/brand-wordmark.png')} style={{ flex: 1, height: 40 }} resizeMode="contain"/>
    ...
    {!route ? <View style={styles.empty}>
      ...
      <View style={styles.mapArea}>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel={t("Получить текущее местоположение")} disabled={!!busy} onPress={locate} style={[styles.iconButton, styles.location]}>{busy === 'gps' ? <ActivityIndicator color="#155847"/> : <Icon name="location"/>}</Pressable>
      </View>
      <View style={styles.panel}>
        ...
        {displayed && <RouteStats title={selected ? t("● Выбранная точка") : current ? t("● Вы на маршруте") : t("Старт маршрута")} color={selected ? '#c26a17' : current ? '#2679e8' : '#155847'} position={displayed}/>}
    ...
    <Modal visible={settings} animationType="slide" onRequestClose={() => setSettings(false)}>
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.settingsContent}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>{(['system', 'ru', 'en', 'de'] as const).map((value) => <Pressable key={value} accessibilityRole="radio" accessibilityState={{ checked: preference === value }} style={[styles.retry, preference === value && { backgroundColor: '#e0efe5' }]} onPress={() => { try { setPreference(value); } catch { Alert.alert(t("Язык не сохранён"), t("Не удалось сохранить язык. Проверьте свободное место.")); } }}><Text style={styles.retryText}>{value === 'system' ? t("Как на устройстве") : { ru: 'Русский', en: 'English', de: 'Deutsch' }[value]}</Text></Pressable>)}</View>
