...
export const ElevationProfile = memo(function ElevationProfile({ route, current, selected, onSelect }: { route: Route; current: RoutePosition | null; selected: RoutePosition | null; onSelect: (distance: number) => void }) {
  ...
  const path = useMemo(() => {
    ...
    for (const segment of route.segments) {
      ...
      for (const p of segment.points) {
        ...
        commands.push(`${move ? 'M' : 'L'}${px.toFixed(2)},${py.toFixed(2)}`); move = false;
      }
    }
    return commands.join(' ');
  }, [route, right, max, min, span]);
  return <View onLayout={(event) => setWidth(event.nativeEvent.layout.width)}>
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}><Text style={{ fontWeight: '700', color: '#173f36' }}>Профиль высоты</Text><Text style={{ color: '#637770', fontSize: 11 }}>Проведите по графику</Text></View>
    <View {...responder.panHandlers} accessible accessibilityRole="adjustable" accessibilityLabel="Выбранная точка на профиле высоты" accessibilityValue={{ min: 0, max: route.totalDistance, now: selected?.distance ?? 0 }} accessibilityActions={[{ name: 'increment', label: 'Вперёд' }, { name: 'decrement', label: 'Назад' }]} onAccessibilityAction={(event) => onSelect((selected?.distance ?? 0) + (event.nativeEvent.actionName === 'increment' ? 1 : -1) * route.totalDistance / 100)}>
      <Svg pointerEvents="none" width={width} height={112}>
        ...
        <SvgText x={0} y={top + 5} fontSize={10} fill="#637770">{Math.round(max)} м</SvgText>
        <SvgText x={0} y={bottom} fontSize={10} fill="#637770">{Math.round(min)} м</SvgText>
        <Path d={path} fill="none" stroke="#197251" strokeWidth={2.5} />
        {route.minElevation === null && <SvgText x={width / 2} y={48} textAnchor="middle" fontSize={12} fill="#637770">В GPX нет высот</SvgText>}
        ...
        <SvgText x={left} y={107} fontSize={10} fill="#637770">0 км</SvgText>
        <SvgText x={right} y={107} textAnchor="end" fontSize={10} fill="#637770">{route.totalDistance.toFixed(1)} км</SvgText>
      </Svg>
    </View>
  </View>;
});
