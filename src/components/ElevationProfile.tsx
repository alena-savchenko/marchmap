...
export const ElevationProfile = memo(function ElevationProfile({ route, current, selected, onSelect }: {
    ...
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}><Text style={{ fontWeight: '700', color: c.text }}>{t("Профиль высоты")}</Text><Text style={{ color: c.muted, fontSize: 11 }}>{t("Проведите по графику")}</Text></View>
    <View {...responder.panHandlers} accessible accessibilityRole="adjustable" accessibilityLabel={t("Выбранная точка на профиле высоты")} accessibilityValue={{ min: 0, max: route.totalDistance, now: selected?.distance ?? 0 }} accessibilityActions={[{ name: 'increment', label: t("Вперёд") }, { name: 'decrement', label: t("Назад") }]} onAccessibilityAction={(event) => onSelect((selected?.distance ?? 0) + (event.nativeEvent.actionName === 'increment' ? 1 : -1) * route.totalDistance / 100)}>
      <Svg pointerEvents="none" width={width} height={112}>
        {[top, (top + bottom) / 2, bottom].map((py) => <Line key={py} x1={left} x2={right} y1={py} y2={py} stroke={c.grid}/>)}
        <SvgText x={0} y={top + 5} fontSize={10} fill={c.muted}>{Math.round(max)}{' '}{t("м")}</SvgText>
        <SvgText x={0} y={bottom} fontSize={10} fill={c.muted}>{Math.round(min)}{' '}{t("м")}</SvgText>
        <Path d={path} fill="none" stroke={c.route} strokeWidth={2.5}/>
        {route.minElevation === null && <SvgText x={width / 2} y={48} textAnchor="middle" fontSize={12} fill={c.muted}>{t("В GPX нет высот")}</SvgText>}
        {current && <Line x1={x(current.distance)} x2={x(current.distance)} y1={top} y2={bottom} stroke={c.blue} strokeWidth={2}/>}
        ...
        <SvgText x={left} y={107} fontSize={10} fill={c.muted}>{t("0 км")}</SvgText>
        <SvgText x={right} y={107} textAnchor="end" fontSize={10} fill={c.muted}>{n(route.totalDistance)}{' '}{t("км")}</SvgText>
      </Svg>
    </View>
  </View>;
});
